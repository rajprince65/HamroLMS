...
function Logo() {
  return (
    <Link href="/" className="flex items-center gap-2 flex-shrink-0">
      ...
      <div>
        <span className="text-[#1D4B6E] font-extrabold text-xl leading-none">
          HamroLMS
        </span>
...
function NavLinkItem({
  ...
  const linkContent = (
    <span className="flex items-center gap-1 px-3 py-2 text-[15px] text-gray-700 hover:text-[#1D8FA0] font-medium rounded-lg transition-colors duration-150 cursor-pointer">
      {item.label}
      {hasChildren && (
        <svg
        ...
        <ul className="absolute top-full left-0 mt-1 min-w-[160px] bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-50">
          {item.children!.map((child) => (
            <li key={child.label}>
              <Link
                href={child.href}
                className="block px-4 py-2.5 text-sm text-gray-700 hover:bg-[#e8f8f5] hover:text-[#1D8FA0] transition-colors"
              >
                {child.label}
              </Link>
            </li>
        ...
        <div className="pl-4 pb-2 flex flex-col gap-1">
          {item.children!.map((child) => (
            <Link
              key={child.label}
              href={child.href}
              className="block py-2 text-gray-600 text-[15px]"
              onClick={onClose}
            >
              {child.label}
            </Link>
...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        ...
        <nav
          ...
          className={`bg-white transition-all duration-500 ${
            scrolled
              ? "w-full rounded-none shadow-md px-4 sm:px-6 py-3"
              : "w-[96%] max-w-7xl rounded-2xl shadow-lg px-4 sm:px-6 py-3"
        ...
        >
          ...
          <div className="hidden lg:flex items-center gap-3">
            <a
              href="#"
              className="bg-[#2DC8A8] hover:bg-[#26b096] text-white text-sm font-semibold px-5 py-2 rounded-lg transition-colors duration-200 whitespace-nowrap"
            >
              Admission Open
            </a>
            <a
              href="#"
              className="bg-[#1D4B6E] hover:bg-[#163a57] text-white text-sm font-semibold px-5 py-2 rounded-lg transition-colors duration-200 flex items-center gap-2 whitespace-nowrap"
            >
              Login
              <svg
      ...
      {/* Drawer panel */}
      <aside
        className={`fixed top-0 right-0 z-[70] h-full w-[82vw] max-w-[360px] bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-in-out lg:hidden ${
